			...
			**/
			if ( $post_type == 'post' ) {
				...
				if ( is_singular() && !$wpex_query ) {
					$width = get_theme_mod( 'wpex_post_img_width', '1200' );
					$height = get_theme_mod( 'wpex_post_img_height', '750' );
				} else {
					// Post Entries
					$width = get_theme_mod( 'wpex_entry_img_width', '640' );
					$height = get_theme_mod( 'wpex_entry_img_height', '400' );
					if ( 'gallery' == get_post_format($post_id) ) {
						$height = get_theme_mod( 'wpex_entry_gallery_img_height', '400' );
					}
				}
			}
